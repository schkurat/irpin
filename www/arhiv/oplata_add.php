...
if($fl_s==1){
		//vtavka v kasu
		$ath=mysql_query("INSERT INTO arhiv_kasa(SZ,NZ,DT,SM,SM_KM) 
		VALUES('$sz','$nz','$dt_opl','$sum','$sum_km')");
		$i++;
		//obnovl v bd zakazov pole datu oplatu 
		if($sum_zak==$ssum){
		$ath=mysql_query("UPDATE arhiv_zakaz SET DOKVUT='$dt_opl' 
...
//		}
		$pr_v=0;
		$sql1="SELECT SUM(SM) AS SM,SUM(SM_KM) AS SM_KM FROM arhiv_s_obr WHERE DL='1' AND D_OBR='$dt_obr'";
					...
					if($pr_v==1){
						$sm=$sm+$sum;
						$sm_km=$sm_km+$sum_km;
						$ath=mysql_query("UPDATE arhiv_s_obr SET SM='$sm',SM_KM='$sm_km'
						WHERE s_obr.DL='1' AND s_obr.D_OBR='$dt_obr'");
					}
					else{
						$sm=$sum;
						$sm_km=$sum_km;
						$ath=mysql_query("INSERT INTO arhiv_s_obr(D_OBR,T_OBR,SM,SM_KM) 
...
echo '<a href="arhiv.php?filter=oplata_info"><input type="button" name="next" value="Наступний"></a>';
?>