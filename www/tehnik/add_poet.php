...
$k=0;
$sql = "SELECT ID,SZ,NZ,NR,NS,VL,BD,KV,POET,POET_BAK FROM tehnik WHERE tehnik.DL='1' AND tehnik.ID_ZAK='$kl_zm'";
$atu=mysql_query($sql);
...
if($_FILES["filename"]["size"] > 1024*3*1024)
   ...
   if(is_uploaded_file($_FILES["filename"]["tmp_name"]))
   {
	 ...
	 $path=$put."/POET/".$sz."_".$nz."_".$_FILES["filename"]["name"];
...
if($_FILES["filename2"]["size"] > 1024*3*1024)
   ...
   if(is_uploaded_file($_FILES["filename2"]["tmp_name"]))
   {
     // Если файл загружен успешно, перемещаем его
     // из временной директории в конечную /home/common/andrey/import
	 $path2=$put."/POET/".$sz."_".$nz."_".$_FILES["filename2"]["name"];
...
if($k==1){$ath1=mysql_query("UPDATE tehnik SET POET='$path',POET_BAK='$path2' WHERE tehnik.ID='$id' AND tehnik.DL='1'");}
else{
$ath1=mysql_query("INSERT INTO tehnik(ID_ZAK,SZ,NZ,KODP,NR,NS,VL,BD,KV,POET,POET_BAK,VUK)
...
header("location: tehnik.php?filter=poet");
?>