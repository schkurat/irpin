...
$nas=$_GET['ns_p'];  
   ...
   $ath=mysql_query("SELECT nas_punktu.ID_NSP,nas_punktu.NSP, nas_punktu.ID_TIP_NSP,tup_nsp.TIP_NSP 
						FROM nas_punktu,tup_nsp WHERE nas_punktu.ID_RN='$rn' AND nas_punktu.ID_NSP='$nas'
						AND nas_punktu.ID_TIP_NSP=tup_nsp.ID_TIP_NSP ORDER BY NSP");
				if($ath)
					{
					while($aut=mysql_fetch_array($ath))
						{
						$pr2.='{value:"'.$aut['ID_NSP'].'",text:"'.$aut['NSP'].' '.$aut['TIP_NSP'].'"}';
						}
					}
