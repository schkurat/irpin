...
if($old_taks==0)
{
$ath=mysql_query("INSERT INTO taks (IDZM,SZ,NZ,DATE_T,NDS,KF) VALUES('$idzm','$sz','$nz','$dt','$pdv','$term');");
if(!$ath){echo "Нова таксировка не внесена до БД";}  

$sql1 = "SELECT LAST_INSERT_ID()";   
$atu1=mysql_query($sql1);   
if($atu1) $iden=mysql_result($atu1,0);
...
case 2:		//robotu arhiv-2
			...
			$t_rob=2;
			break;
case 3:		//robotu tehnika-4,vumiryuvacha-3,brigadira-5
			$s[$i]=round((round(($n_vuk[$i]*$n_god),2)*$kl[$i]*$skl[$i]),2);
			...
			//$rob=$teh;
			$rob=$ispoln[$i];
			$rob_vum=$ispoln[$i];//$vum;
			$rob_kont=$brug;
			$t_rob=4;
			$t_rob_vm=3;
			$t_rob_br=5;
			break;
case 4:		//robotu buro-2
			$s[$i]=round((round(($n_vuk[$i]*$n_god),2)*$kl[$i]*$skl[$i]),2);
			$s_buro=$s_buro+$s[$i];
			$rob=1;
			$t_rob=6;
...
if($n_vuk[$i]!=0){
 $ath1=mysql_query("INSERT INTO taks_detal (ID_TAKS,IDZM,SZ,NZ,ID_ROB,ID_PRICE,TR,NORM,KL,KF_SKL,SUM,NDS,KF,KF_NAR) 
	VALUES('$iden','$idzm','$sz','$nz','$rob','$price[$i]','$t_rob','$n_vuk[$i]','$kl[$i]','$skl[$i]','$s[$i]','$pdv','$term','$kf_nar');");}
//------------------------------------------------------------------------------------------------------
//------------------------------------vumiruvach--------------------------------------------------------
if($n_vum[$i]!=0 and $vum!=0){
 $ath1=mysql_query("INSERT INTO taks_detal (ID_TAKS,IDZM,SZ,NZ,ID_ROB,ID_PRICE,TR,NORM,KL,KF_SKL,SUM,NDS,KF,KF_NAR) 
	VALUES('$iden','$idzm','$sz','$nz','$rob_vum','$price[$i]','$t_rob_vm','$n_vum[$i]','$kl[$i]','$skl[$i]','$svm[$i]','$pdv','$term','$kf_nar');");}
//------------------------------------------------------------------------------------------------------
//------------------------------------kontroler---------------------------------------------------------
if($n_kontr[$i]!=0 and $brug!=0){
 $ath1=mysql_query("INSERT INTO taks_detal (ID_TAKS,IDZM,SZ,NZ,ID_ROB,ID_PRICE,TR,NORM,KL,KF_SKL,SUM,NDS,KF,KF_NAR) 
	VALUES('$iden','$idzm','$sz','$nz','$rob_kont','$price[$i]','$t_rob_br','$n_kontr[$i]','$kl[$i]','$skl[$i]','$skon[$i]','$pdv','$term','$kf_nar');");}
//------------------------------------------------------------------------------------------------------
}
}
if($vum==0) $s_vum=0;
...
//-----------------------------------------
$ath2=mysql_query("UPDATE taks SET SUM='$vart',SUM_OKR='$vr_per2' WHERE ID_TAKS='$iden'");
