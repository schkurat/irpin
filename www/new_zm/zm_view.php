...
if ($szak != "" and $nzak != "") {
    $flag = "zamovlennya.SZ=" . $szak . " AND zamovlennya.NZ=" . $nzak;
}
...
while ($aut = mysql_fetch_array($atu)) {
    ...
    if ($ddl == '1') {
        if ($aut["PS"] == '0') {
            $vst_bl = '<td align="center"><a href="index.php?filter=delete_info&tip=zakaz&kl=' . $aut["KEY"] . '"><img src="../images/b_drop.png" border="0"></a></td>';
        } else {
...
//if($aut["RAH"]!='0' || $aut["TUP_ZAM"]!=2){
    $kvut = '<a href="print_kvut_zm.php?kl=' . $aut["KEY"] . '&tz=' . $aut["TUP_ZAM"] . '"><img src="../images/kvut.png" border="0"></a>';
    /* }
    else{
    $kvut='-';
    } */
    $adr_storage = urlencode(serialize($aut["TIP_NSP"] . $aut["NSP"] . " " . $aut["TIP_VUL"] . $aut["VUL"] . " " . $obj_ner));
...
<td align="center"><a href="index.php?filter=storage&url=' . $aut["EA"] . '/document&parent_link=' . $parent_link . '&adr=' . $adr_storage . '"><img src="../images/database.png" border="0"></a></td>	
      ...
      <td align="center" id="zal"><a href="index.php?filter=zmina_info&fl=prizv&kl=' . $aut["KEY"] . '">' . $aut["PR"] . " " . $aut["IM"] . " " . $aut["PB"] . '</a></td>
	  <td align="center">' . $aut["PRVL"] . " " . $aut["IMVL"] . " " . $aut["PBVL"] . '</td>
	  <td align="center" id="zal"><a href="index.php?filter=zmina_info&fl=prim&kl=' . $aut["KEY"] . '">' . $aut["PRIM"] . '</a></td>
	  <td align="center">' . $aut["TEL"] . '</td>
	  <!--<td align="center" id="zal"><a href="index.php?filter=zmina_info&fl=email&kl=' . $aut["KEY"] . '">' . $mulo . '</a></td>-->
      <td align="center" id="zal"><a href="index.php?filter=zmina_info&fl=adres&kl=' . $aut["KEY"] . '">
				' . $aut["TIP_NSP"] . $aut["NSP"] . " " . $aut["TIP_VUL"] . $aut["VUL"] . " " . $obj_ner . '</a></td>
	  <td align="center" id="zal"><a href="index.php?filter=zmina_info&fl=vartist&kl=' . $aut["KEY"] . '">' . $aut["SUM"] . '</a></td>
