<?php
$name = $_GET['name'];
$sql = "SELECT PR,IM,PB,PRD,DRN,DPS,DTL FROM security WHERE LOG='$name' AND DL='1'";
$atu = mysql_query($sql);
...
<form action="update_user.php" name="myform" method="post">
    <table align="center" class="zmview">
        <tr>
            <th colspan="4" align="center">Редагування користувача</th>
        </tr>
        <tr>
            <td colspan="4">Прізвище <input name="priz" type="text" value="<?php echo $pr; ?>"/>
                Ім`я<input name="imya" type="text" value="<?php echo $im; ?>"/>
                По батькові<input name="pobat" type="text" value="<?php echo $pb; ?>"/></td>
        </tr>
        <tr>
            <th colspan="4">Права доступу</th>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            <th colspan="4">Доступ до районів</th>
        </tr>
        <?php
        $kk = 0;
        $sql = "SELECT * FROM rayonu ORDER BY RAYON";
        $atu = mysql_query($sql);
        while ($aut = mysql_fetch_array($atu)) {
            $kk++;
            ...
            if ($kk == 4) {
                $ss .= '<td><input name="drn' . $aut["ID_RAYONA"] . '" type="checkbox" value="' . $aut["ID_RAYONA"] . '" ' . $mas2[$aut["ID_RAYONA"]] . '/>' . $n_rjn . '</td></tr>';
                $kk = 0;
            }
            if ($kk == 1) {
                $ss .= '<tr><td><input name="drn' . $aut["ID_RAYONA"] . '" type="checkbox" value="' . $aut["ID_RAYONA"] . '" ' . $mas2[$aut["ID_RAYONA"]] . '/>' . $n_rjn . '</td>';
            }
            if ($kk != 1 and $kk != 4 and $kk != 0) {
                $ss .= '<td><input name="drn' . $aut["ID_RAYONA"] . '" type="checkbox" value="' . $aut["ID_RAYONA"] . '" ' . $mas2[$aut["ID_RAYONA"]] . '/>' . $n_rjn . '</td>';
            }
        }
        mysql_free_result($atu);
        if ($kk <= 4) {
        ...
        ?>
        <tr>
            <th colspan="4">Розширені права доступу</th>
        </tr>
        <tr>
            <td>
