...
<form action="add_user_db.php" name="myform" method="post">
    <table align="center" class="zmview">
        <tr>
            <th colspan="4" align="center">Новий користувач</th>
        </tr>

        <tr>
            <td colspan="4">Прізвище <input name="priz" type="text"/>
                Ім`я<input name="imya" type="text"/>
                По батькові<input name="pobat" type="text"/></td>
        </tr>
        <tr>
            <td colspan="4">Логін &nbsp &nbsp &nbsp &nbsp<input name="log" type="text"/></td>
        </tr>
        <tr>
            <td colspan="4">Пароль &nbsp &nbsp <input id="parol" name="pas" type="text"/>
                Пароль ще раз<input name="pas2" type="text"/></td>
        </tr>
        <tr>
        ...
        while ($aut = mysql_fetch_array($atu)) {
            ...
            if ($kk == 4) {
                $ss .= '<td><input name="drn' . $aut["ID_RAYONA"] . '" type="checkbox" value="' . $aut["ID_RAYONA"] . '"/>' . $n_rjn . '</td></tr>';
                $kk = 0;
            }
            if ($kk == 1) {
                $ss .= '<tr><td><input name="drn' . $aut["ID_RAYONA"] . '" type="checkbox" value="' . $aut["ID_RAYONA"] . '"/>' . $n_rjn . '</td>';
            }
            if ($kk != 1 and $kk != 4 and $kk != 0) {
                $ss .= '<td><input name="drn' . $aut["ID_RAYONA"] . '" type="checkbox" value="' . $aut["ID_RAYONA"] . '"/>' . $n_rjn . '</td>';
            }
        }
